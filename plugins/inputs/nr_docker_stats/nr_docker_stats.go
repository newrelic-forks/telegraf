...
func (d *Docker) Gather(acc telegraf.Accumulator) error {
	...
	// Get CPU count from daemon info endpoint to work around incorrect
	// cpu count in /sys/fs/cgroup/cpu/docker/cpuacct.usage_percpu
	// See https://github.com/docker/docker/issues/26591
	CPUCount, err := d.getCPUCount()
	if err != nil {
		return fmt.Errorf("Error getting cpu count: %s", err.Error())
	}
	d.cpuCount = CPUCount
...
func (d *Docker) gatherInfo(acc telegraf.Accumulator) error {
	...
	// Add metrics
	acc.AddFields("docker_daemon_info",
		fields,
		map[string]string{"engine_host": d.engine_host},
		now)
	acc.AddFields("docker_daemon_mem",
...
func (d *Docker) gatherContainerStats(
	...
	memfields := map[string]interface{}{
		"mem_max_usage":                 stat.MemoryStats.MaxUsage,
		"mem_usage":                     stat.MemoryStats.Usage,
		"mem_fail_count":                stat.MemoryStats.Failcnt,
		"mem_limit":                     stat.MemoryStats.Limit,
		"mem_total_pgmafault":           stat.MemoryStats.Stats["total_pgmajfault"],
		"mem_cache":                     stat.MemoryStats.Stats["cache"],
		"mem_mapped_file":               stat.MemoryStats.Stats["mapped_file"],
		"mem_total_inactive_file":       stat.MemoryStats.Stats["total_inactive_file"],
		"mem_pgpgout":                   stat.MemoryStats.Stats["pagpgout"],
		"mem_rss":                       stat.MemoryStats.Stats["rss"],
		"mem_total_mapped_file":         stat.MemoryStats.Stats["total_mapped_file"],
		"mem_writeback":                 stat.MemoryStats.Stats["writeback"],
		"mem_unevictable":               stat.MemoryStats.Stats["unevictable"],
		"mem_pgpgin":                    stat.MemoryStats.Stats["pgpgin"],
		"mem_total_unevictable":         stat.MemoryStats.Stats["total_unevictable"],
		"mem_pgmajfault":                stat.MemoryStats.Stats["pgmajfault"],
		"mem_total_rss":                 stat.MemoryStats.Stats["total_rss"],
		"mem_total_rss_huge":            stat.MemoryStats.Stats["total_rss_huge"],
		"mem_total_writeback":           stat.MemoryStats.Stats["total_write_back"],
		"mem_total_inactive_anon":       stat.MemoryStats.Stats["total_inactive_anon"],
		"mem_rss_huge":                  stat.MemoryStats.Stats["rss_huge"],
		"mem_hierarchical_memory_limit": stat.MemoryStats.Stats["hierarchical_memory_limit"],
		"mem_total_pgfault":             stat.MemoryStats.Stats["total_pgfault"],
		"mem_total_active_file":         stat.MemoryStats.Stats["total_active_file"],
		"mem_active_anon":               stat.MemoryStats.Stats["active_anon"],
		"mem_total_active_anon":         stat.MemoryStats.Stats["total_active_anon"],
		"mem_total_pgpgout":             stat.MemoryStats.Stats["total_pgpgout"],
		"mem_total_cache":               stat.MemoryStats.Stats["total_cache"],
		"mem_inactive_anon":             stat.MemoryStats.Stats["inactive_anon"],
		"mem_active_file":               stat.MemoryStats.Stats["active_file"],
		"mem_pgfault":                   stat.MemoryStats.Stats["pgfault"],
		"mem_inactive_file":             stat.MemoryStats.Stats["inactive_file"],
		"mem_total_pgpgin":              stat.MemoryStats.Stats["total_pgpgin"],
		"mem_usage_percent":             calculateMemPercent(stat),
	}
	...
	cpufields := map[string]interface{}{
		...
		"cpu_usage_in_kernelmode":          stat.CPUStats.CPUUsage.UsageInKernelmode,
		"cpu_usage_system":                 stat.CPUStats.SystemUsage,
	...
	for _, netstats := range stat.Networks {
		netfields := map[string]interface{}{
			"net_rx_dropped": netstats.RxDropped,
			"net_rx_bytes":   netstats.RxBytes,
			"net_rx_errors":  netstats.RxErrors,
			"net_tx_packets": netstats.TxPackets,
			"net_tx_dropped": netstats.TxDropped,
			"net_rx_packets": netstats.RxPackets,
			"net_tx_errors":  netstats.TxErrors,
			"net_tx_bytes":   netstats.TxBytes,
		}
	...
	if total && len(totalNetworkStatMap) != 0 {
		...
		nettags["container_id"] = id
		acc.AddFields("docker_container_net", totalNetworkStatMap, nettags, now)
	}

	gatherBlockIOMetrics(stat, acc, tags, now, id, total)
}
...
func calculateCoreCount(stat *types.StatsJSON, cpuCount int) float64 {
	var coreCount = 0.0
	...
	if systemDelta > 0.0 && cpuDelta > 0.0 {
		coreCount = (cpuDelta / systemDelta) * float64(cpuCount)
	}
	return coreCount
}
...
func init() {
	inputs.Add("nr_docker_stats", func() telegraf.Input {
		return &Docker{
